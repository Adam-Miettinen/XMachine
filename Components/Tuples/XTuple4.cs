...
{
	internal sealed class XTuple<T1, T2, T3, T4> : XTypeComponent<Tuple<T1, T2, T3, T4>>
	{
		internal XTuple() { }

		protected override void OnBuild(XType<Tuple<T1, T2, T3, T4>> xType, IXReadOperation reader, XElement element,
			ObjectBuilder<Tuple<T1, T2, T3, T4>> objectBuilder)
		{
			// T1
			...
			reader.AddTask(this, () =>
			{
				...
				{
					objectBuilder.Object = new Tuple<T1, T2, T3, T4>(item1, item2, item3, item4);
					return true;
				}
				return false;
			});
		...
		protected override bool OnWrite(XType<Tuple<T1, T2, T3, T4>> xType, IXWriteOperation writer,
			Tuple<T1, T2, T3, T4> obj, XElement element)
		{
